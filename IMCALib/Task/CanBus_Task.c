...
moto_measure_t   Friction_Motor[2];  //底盘电机参数结构体
moto_measure_t   Pluck_Motor[1];	//拨弹轮电机参数结构体
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *_hcan)
{
	...
	switch(rx_header.StdId)
    {
        ...
        {
            static u8 j;
            j = rx_header.StdId - CAN_2006Moto1_ID;
            
            Pluck_Motor[j].msg_cnt++ <= 50 ? GetMotorOffset(&Pluck_Motor[j], rx_data) : GetMotorMeasure(&Pluck_Motor[j], rx_data);  /*读取拨弹电机参数信息*/
		...
		case CAN_3508Moto2_ID:
        {
            ...
            Friction_Motor[i].msg_cnt++ <= 50 ? GetMotorOffset(&Friction_Motor[i], rx_data) : GetMotorMeasure(&Friction_Motor[i], rx_data);  /*读取摩擦轮电机参数信息*/
        ...
        {
            static u8 k;
            k = rx_header.StdId - CAN_YAW_Motor_ID;            
            Gimbal_Motor[k].msg_cnt++ <= 50 ? GetMotorOffset(&Gimbal_Motor[k], rx_data) : GetMotorMeasure(&Gimbal_Motor[k], rx_data);  /*读取云台电机参数信息*/
 ...
 *         此函数是对应前4个ID的电调
**********************************************************************************************************************/
void SetPluckMotorCurrent(CAN_HandleTypeDef *hcan,s16 iq1, s16 iq2, s16 iq3, s16 iq4)
{
 ...
 *         此函数是对应后4个ID的电调
**********************************************************************************************************************/
void SetChassisAndGimbalMotorValue(CAN_HandleTypeDef *hcan,s16 iq1, s16 iq2, s16 iq3, s16 iq4)
{
