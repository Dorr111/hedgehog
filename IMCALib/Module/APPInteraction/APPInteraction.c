 ...
 * 使用方法 ：开启USART3的中断，波特率为115200，同时要开通一个定时器，设置频率为10hz，100ms中断一次，用于执行usmart扫描。
...
void APPInteractionInit(void)
{
    ...
    HAL_UART_Receive_IT(&huart3, (u8 *)aRxBuffer, RXBUFFERSIZE);//该函数会开启串口1接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量
...
/**********************************************************************************************************************
 * @brief   超时处理，重启串口3接收中断，在USART3_IRQHandler()中被调用
 * @param   None
 * @retval  None
 * @others  USART3的中断优先级和定时器7的中断优先级最好设置成一样
**********************************************************************************************************************/
void Reset_Usart3_Receive_IT(void) 
{
    u32 timeout=0;
    timeout=0;
